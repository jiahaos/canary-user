...
public class TreeUtil {
     ...
     * @return 树形结构集合
     */
    public List<AclPermissionDTO> getChildAclPermission(List<AclPermissionDTO> list, Integer parentId) {
        List<AclPermissionDTO> returnList = new ArrayList<AclPermissionDTO>();
        for (Iterator<AclPermissionDTO> iterator = list.iterator(); iterator.hasNext(); ) {
            AclPermissionDTO t = iterator.next();
            // 一、根据传入的某个父节点ID,遍历该父节点的所有子节点
            if (t.getParentId() == parentId) {
                recursionFn(list, t);
                returnList.add(t);
     ...
     * @param t
     */
    private void recursionFn(List<AclPermissionDTO> list, AclPermissionDTO t) {
        List<AclPermissionDTO> childList = getChildList(list, t);// 得到子节点列表
        t.setChildren(childList);
        for (AclPermissionDTO tChild : childList) {
            if (hasChild(list, tChild)) {// 判断是否有子节点
                Iterator<AclPermissionDTO> it = childList.iterator();
                while (it.hasNext()) {
                    AclPermissionDTO n = (AclPermissionDTO) it.next();
                    recursionFn(list, n);
                }
            }
        }
     ...
     * @return
     */
    private List<AclPermissionDTO> getChildList(List<AclPermissionDTO> list, AclPermissionDTO t) {
        List<AclPermissionDTO> tlist = new ArrayList<AclPermissionDTO>();
        Iterator<AclPermissionDTO> it = list.iterator();
        while (it.hasNext()) {
            AclPermissionDTO n = (AclPermissionDTO) it.next();
            if (t.getType() != 2) {
                if (n.getParentId() == Integer.valueOf(t.getId().toString())) {
     ...
     * @return
     */
    private boolean hasChild(List<AclPermissionDTO> list, AclPermissionDTO t) {
        return getChildList(list, t).size() > 0 ? true : false;
    }
